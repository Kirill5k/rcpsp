...
public enum Algorithms {
    ...
    public List<EventList> run(BenchmarkInstance bi) {
        LOG.info("Running benchmark instance {} on {}. Best known solution {}. Critical path length {}", bi.getName(), this, BenchmarkFactory.solutions.get(bi.getName()), CommonOperations.getCriticalPathLength(bi));
        return run(ProjectFactory.asRandomEventList(bi));
    }

    public List<EventList> run(EventList el) {
        Algorithm alg;
        switch (this){
            ...
            default: throw new IncorrectAlgorithmTypeException(this + " is not supported");
        }

        return solve(alg);
    }

    private List<EventList> solve(Algorithm alg) {
